...
class FoodCollector():
    ...
    def reset(self):
        ...
        for poison in self._poisons:
            while (True):
                ...
                collision = False
                ### Calculate the distance to all the evaders
                for evader in self._evaders:
                    distance = ssd.cdist(np.array([poison.position]), 
                                         np.array([evader.position]))
                    if distance <= 1.2 * (evader._radius + poison._radius):
                        collision = True
                if not collision:
                    break
    ...
    def collision_handling_subroutine(self, rewards, is_last):
                    ...
                    if poison:
                        ...
                        while (True):
                            ...
                            if not collision:
                                ...
                                poison.set_position(
                                self._generate_coord(poison._radius))
        ...
        if self.food_revive:
            reset_caught_objects(caught_evaders, self._evaders, 
                                 self.evader_speed, poison=False)
        reset_caught_objects(caught_poisons, self._poisons, 
                             self.poison_speed, evaders=self._evaders, 
                             poison=True)
